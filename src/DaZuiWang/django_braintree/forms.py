...
from braintree import MerchantAccount,WebhookNotification
...
class SubmerchantForm(forms.ModelForm):
    class Meta:
        model = models.MerchantCount
        fields=[
                'MerchantName',
                'm_firstname',
                'm_last_name',
    ...
    def save(self):
        m_applicant_details = {    # cc details
            'first_name': Meta.m_firstname,
            'last_name': self.cleaned_data['name'],
            'email': '%s/%s' %\
                (self.cleaned_data['expiration_month'], self.cleaned_data['expiration_year']),
            'phone': self.cleaned_data['cvv'],
            'address': {
                'street_address': self.cleaned_data['zip_code'],
                "postal_code":'',
                "locality":'',
                "region":'',
            },
        ...
        result = MerchantAccount.create({
              "applicant_details": m_applicant_details,
              "tos_accepted": True,
...
class UserCCDetailsForm(forms.Form):
    ...
    def save(self, prepend_vault_id=''):
        ...
        print self.cleaned_data['cc_number']
